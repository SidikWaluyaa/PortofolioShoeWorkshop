                ...
                @if (is_array($element))
                    @foreach ($element as $page => $url)
                        @if ($page == $paginator->currentPage())
                            <span class="w-10 h-10 flex items-center justify-center rounded-xl bg-[#22AF85] text-white text-sm font-extrabold shadow-md shadow-[#22AF85]/20 select-none">
                                {{ $page }}
                            </span>
                        @else
                            <a href="{{ $url }}" 
                               class="pagination-link w-10 h-10 flex items-center justify-center rounded-xl bg-white border border-gray-200 text-gray-600 hover:bg-[#22AF85] hover:text-white hover:border-[#22AF85] active:scale-95 transition-all text-sm font-bold shadow-sm">
                                {{ $page }}
                            </a>
                        @endif
                    @endforeach
