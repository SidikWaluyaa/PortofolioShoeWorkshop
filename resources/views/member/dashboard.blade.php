<x-member-layout>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            <div class="flex items-center justify-between mb-6">
                ...
                <a href="{{ route('member.checkin.index') }}" class="text-sm font-semibold text-emerald-600 hover:text-emerald-700">Lihat Semua →</a>
            </div>
            ...
            @if($streakStatus['streak_complete'] && $streakStatus['can_claim'])
                <div class="mt-6 p-4 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-500 text-white text-center">
                    ...
                    <a href="{{ route('member.rewards.index') }}" class="inline-block mt-2 px-4 py-2 bg-white text-emerald-600 font-bold text-xs rounded-lg hover:bg-gray-50 transition">Klaim Reward →</a>
                </div>
            @endif
        </div>
    ...
    <div class="mt-8 bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        <div class="flex items-center justify-between mb-6">
            ...
            <a href="{{ route('member.donations.index') }}" class="text-sm font-semibold text-emerald-600 hover:text-emerald-700">Lihat Semua →</a>
        </div>
        @if($donations->isEmpty())
            <div class="text-center py-8">
                <p class="text-sm text-gray-500">Belum ada donasi. Mulai donasikan sepatu Anda!</p>
                <a href="{{ route('member.donations.create') }}" class="inline-block mt-3 px-4 py-2 bg-emerald-500 text-white font-bold text-xs rounded-lg hover:bg-emerald-600 transition">+ Donasi Sepatu</a>
            </div>
        @else
