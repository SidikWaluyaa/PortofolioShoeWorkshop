                ...
                <form action="{{ route('member.donations.store') }}" method="POST" enctype="multipart/form-data" 
                    ...
                    <div class="mb-6">
                        ...
                        <div class="border-2 border-dashed border-gray-300 rounded-xl p-6 hover:border-emerald-400 transition relative bg-gray-50/50">
                            <!-- Previews Grid -->
                            <div class="grid grid-cols-3 sm:grid-cols-4 gap-4" x-show="uploadedFiles.length > 0">
                                <template x-for="(fileObj, index) in uploadedFiles" :key="fileObj.id">
                                    <div class="relative aspect-square rounded-lg overflow-hidden border border-gray-200 bg-white group shadow-sm">
                                        ...
                                        <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                            <button type="button" @click.stop="removeFile(index)" class="p-1.5 bg-red-600 hover:bg-red-700 text-white rounded-full transition shadow-md">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                                            </button>
                                        </div>
                                        
                                        <!-- Tag/Number -->
                                        <div class="absolute bottom-1 left-1 bg-gray-900/75 text-[9px] text-white px-1.5 py-0.5 rounded font-bold">
                            ...
                            <div class="text-center py-4 cursor-pointer" x-show="uploadedFiles.length === 0" @click="triggerInput()">
                        ...
                        <div class="flex items-center justify-between mt-2.5">
                            ...
                            <span class="text-xs text-gray-400 font-bold" x-text="uploadedFiles.length + ' foto terpilih'"></span>
                        </div>

                        <!-- Hidden files inputs -->
                        <input type="file" x-ref="fileInput" accept="image/*" class="hidden" multiple @change="handleFiles($event)">
                        <input type="file" name="foto[]" x-ref="realInput" id="foto" class="hidden" multiple :required="uploadedFiles.length === 0">
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <a href="{{ route('member.donations.index') }}" class="px-6 py-3 text-sm font-bold text-gray-500 hover:text-gray-700 transition">Batal</a>
                    </div>
                </form>
            </div>
        </div>
