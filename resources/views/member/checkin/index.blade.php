<x-member-layout>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-8 mb-8">
        ...
        @if($streakStatus['streak_complete'] && $streakStatus['can_claim'])
            <div class="p-5 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-500 text-white text-center">
                ...
                <a href="{{ route('member.rewards.index') }}" class="inline-block mt-3 px-6 py-2.5 bg-white text-emerald-600 font-bold text-sm rounded-xl hover:bg-gray-50 transition">🎁 Klaim Reward</a>
            </div>
        @endif
    </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-8">
        ...
        @else
            <form action="{{ route('member.checkin.store') }}" method="POST" enctype="multipart/form-data" x-data="{ preview: null }">
                @csrf
                <div class="mb-6">
    ...
    <div class="mt-8 bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-4">
            ...
            <div class="rounded-xl border {{ $checkin->status === 'approved' ? 'border-emerald-200 bg-emerald-50' : ($checkin->status === 'rejected' ? 'border-red-200 bg-red-50' : 'border-amber-200 bg-amber-50') }} p-3 text-center">
                ...
                <p class="text-[10px] text-gray-500">{{ \Carbon\Carbon::parse($checkin->tanggal_checkin)->format('d M') }}</p>
        ...
        for ($row = 0; $row < 6; $row++) {
            for ($col = 1; $col <= 7; $col++) {
                if ($row === 0 && $col < $startDayOfWeek) {
                    $calendarGrid[$row][] = null;
                } elseif ($currentDay <= $daysInMonth) {
                    $calendarGrid[$row][] = $currentDay;
                    $currentDay++;
                } else {
                    $calendarGrid[$row][] = null;
                }
    ...
    <div class="mt-8 bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-8">
        <div class="flex items-center justify-between mb-6">
            <h3 class="text-lg font-bold text-gray-900">Kalender Kehadiran</h3>
            <span class="px-3 py-1 bg-gray-50 text-gray-700 text-sm font-bold rounded-lg">{{ $now->translatedFormat('F Y') }}</span>
        </div>
                ...
                @if(array_filter($row))
                    <div class="grid grid-cols-7 gap-2 sm:gap-4">
                        @foreach($row as $day)
                            @if($day)
                                @php
                                    $dateString = $now->copy()->day($day)->toDateString();
                                    $isCheckedIn = in_array($dateString, $calendarData ?? []);
                                    $isToday = $day === $now->day;
                                @endphp
                                <div class="relative flex flex-col items-center justify-center h-10 sm:h-12 rounded-xl border-2 transition-all
                                    {{ $isToday ? 'border-[#22AF85] shadow-sm' : 'border-transparent bg-gray-50' }}
                                    {{ $isCheckedIn ? 'bg-emerald-100/50' : '' }}
                                ">
                                    <span class="text-sm sm:text-base font-bold {{ $isCheckedIn ? 'text-emerald-700' : 'text-gray-500' }}">{{ $day }}</span>
                            ...
                            @else
                                <div class="h-10 sm:h-12 bg-transparent"></div>
                            @endif
                        @endforeach
                    </div>
                @endif
        ...
        <div class="mt-6 flex flex-wrap items-center gap-4 text-xs text-gray-500 font-medium">
            <div class="flex items-center gap-1.5"><div class="w-3 h-3 rounded-full bg-emerald-500"></div> Sudah Check-In</div>
            <div class="flex items-center gap-1.5"><div class="w-3 h-3 rounded-xl bg-gray-100"></div> Belum / Bolong</div>
            <div class="flex items-center gap-1.5"><div class="w-4 h-4 rounded-xl border-2 border-[#22AF85]"></div> Hari Ini</div>
        </div>
    </div>
</x-member-layout>
