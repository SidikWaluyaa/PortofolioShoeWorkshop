<x-app-layout>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 sm:p-8 max-w-4xl" x-data="formApp()">
        <form action="{{ route('admin.donation-items.update', $donationItem) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PATCH')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" id="nama" name="nama" value="{{ old('nama', $donationItem->nama) }}" required placeholder="Contoh: Nike Air Max 90"
                ...
                <div>
                    ...
                    <input type="text" id="brand" name="brand" value="{{ old('brand', $donationItem->brand) }}" placeholder="Contoh: Nike (kosongkan jika tidak ada)"
                           class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#22AF85]/20 focus:border-[#22AF85] text-sm text-gray-900 font-medium transition" />
                    @error('brand')
                        <p class="text-xs text-red-500 font-semibold mt-1">{{ $message }}</p>
                    @enderror
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
                ...
                <div>
                    ...
                    <select id="kategori" name="kategori" required
                        ...
                        <option value="tas" {{ old('kategori', $donationItem->kategori) === 'tas' ? 'selected' : '' }}>🎒 Tas</option>
                        <option value="topi" {{ old('kategori', $donationItem->kategori) === 'topi' ? 'selected' : '' }}>🧢 Topi</option>
                    </select>
                    @error('kategori')
                        <p class="text-xs text-red-500 font-semibold mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select id="kondisi" name="kondisi" required
                            class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#22AF85]/20 focus:border-[#22AF85] text-sm text-gray-900 font-semibold transition">
                        <option value="baru" {{ old('kondisi', $donationItem->kondisi) === 'baru' ? 'selected' : '' }}>Baru</option>
                ...
                <div>
                    ...
                    <input type="text" id="ukuran" name="ukuran" value="{{ old('ukuran', $donationItem->ukuran) }}" placeholder="Contoh: US 10.5, Medium, All Size"
                           class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#22AF85]/20 focus:border-[#22AF85] text-sm text-gray-900 font-medium transition" />
                    @error('ukuran')
                        <p class="text-xs text-red-500 font-semibold mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select id="status" name="status" required
                        ...
                        <option value="arsip" {{ old('status', $donationItem->status) === 'arsip' ? 'selected' : '' }}>Arsip</option>
            ...
            <div class="border-t border-gray-100 pt-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="number" id="berat" name="berat" value="{{ old('berat', $donationItem->berat) }}" min="0" placeholder="Contoh: 850 (untuk 0.85 kg)"
                               class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#22AF85]/20 focus:border-[#22AF85] text-sm text-gray-900 font-medium transition" />
                        @error('berat')
                            <p class="text-xs text-red-500 font-semibold mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="number" id="score_kelayakan" name="score_kelayakan" value="{{ old('score_kelayakan', $donationItem->score_kelayakan) }}" min="0" max="100" placeholder="Contoh: 90"
                               class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#22AF85]/20 focus:border-[#22AF85] text-sm text-gray-900 font-medium transition" />
                ...
                <div class="space-y-3">
                    ...
                    <div class="relative flex items-center justify-center border-2 border-dashed border-gray-200 hover:border-[#22AF85] rounded-2xl aspect-square max-h-[300px] w-full max-w-[300px] bg-gray-50 hover:bg-[#22AF85]/5 transition overflow-hidden group">
                        ...
                        <div class="text-center p-4">
                            ...
                            <p class="text-[10px] text-gray-400 mt-1" x-text="detailPreviews.length > 0 ? `${detailPreviews.length} foto baru siap ditambahkan` : 'Klik untuk pilih foto'"></p>
                        </div>
                    </div>
                    @error('foto_detail')
                        <p class="text-xs text-red-500 font-semibold mt-1">{{ $message }}</p>
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-3">
                            <p class="text-xs font-black text-gray-700 uppercase tracking-wider">
                                Foto Detail
                                <span class="ml-1 font-normal text-gray-400" x-text="`(${currentDetails.length + detailPreviews.length} foto)`"></span>
                            </p>
                        ...
                        <div class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3">
                            ...
                            <template x-for="(url, index) in currentDetails" :key="'old-' + index">
                                <div class="relative rounded-xl border border-gray-200 aspect-square overflow-hidden group">
                                    ...
                                    <span class="absolute bottom-1 left-1 bg-black/50 text-white text-[9px] font-bold px-1.5 py-0.5 rounded"
                                          x-text="'#' + (index + 1)"></span>
                                </div>
                            </template>
                            ...
                            <template x-for="(url, index) in detailPreviews" :key="'new-' + index">
                                <div class="relative rounded-xl border border-gray-200 aspect-square overflow-hidden group">
                                    ...
                                    <span class="absolute bottom-1 left-1 bg-[#22AF85] text-white text-[9px] font-bold px-1.5 py-0.5 rounded">Baru</span>
                                </div>
                            </template>

                        </div>
                        {{-- Notif foto yang akan dihapus --}}
                        <p x-show="deletedIndices.length > 0" class="text-xs text-amber-600 font-semibold mt-3 flex items-center gap-1.5">
