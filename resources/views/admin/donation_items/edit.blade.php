<x-app-layout>
    <x-slot name="header">Edit Barang Katalog Donasi</x-slot>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 sm:p-8 max-w-4xl" x-data="formApp()">
        <form action="{{ route('admin.donation-items.update', $donationItem) }}" method="POST"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" id="nama" name="nama" value="{{ old('nama', $donationItem->nama) }}" required
                ...
                <div>
                    <label for="brand"
                        ...
                        Merek</label>
                    <input type="text" id="brand" name="brand" value="{{ old('brand', $donationItem->brand) }}"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
                ...
                <div>
                    ...
                    <select id="kategori" name="kategori" required
                        ...
                        <option value="sepatu" {{ old('kategori', $donationItem->kategori) === 'sepatu' ? 'selected' : '' }}>👞 Sepatu</option>
                        <option value="tas" {{ old('kategori', $donationItem->kategori) === 'tas' ? 'selected' : '' }}>🎒
                            Tas</option>
                        <option value="topi" {{ old('kategori', $donationItem->kategori) === 'topi' ? 'selected' : '' }}>
                            🧢 Topi</option>
                    </select>
                    @error('kategori')
                        <p class="text-xs text-red-500 font-semibold mt-1">{{ $message }}</p>
                ...
                <div>
                    ...
                    <select id="kondisi" name="kondisi" required
                        class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#22AF85]/20 focus:border-[#22AF85] text-sm text-gray-900 font-semibold transition">
                        <option value="baru" {{ old('kondisi', $donationItem->kondisi) === 'baru' ? 'selected' : '' }}>
                            Baru</option>
                        <option value="seperti_baru" {{ old('kondisi', $donationItem->kondisi) === 'seperti_baru' ? 'selected' : '' }}>Like New</option>
                        <option value="sudah_diperbaiki" {{ old('kondisi', $donationItem->kondisi) === 'sudah_diperbaiki' ? 'selected' : '' }}>Refurbished</option>
                    </select>
                    @error('kondisi')
                        <p class="text-xs text-red-500 font-semibold mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="text" id="ukuran" name="ukuran" value="{{ old('ukuran', $donationItem->ukuran) }}"
                ...
                <div>
                    ...
                    <input type="text" id="warna" name="warna" value="{{ old('warna', $donationItem->warna) }}"
                ...
                <div>
                    ...
                    <select id="status" name="status" required
                        class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#22AF85]/20 focus:border-[#22AF85] text-sm text-gray-900 font-semibold transition">
                        <option value="tersedia" {{ old('status', $donationItem->status) === 'tersedia' ? 'selected' : '' }}>Tersedia</option>
                        <option value="arsip" {{ old('status', $donationItem->status) === 'arsip' ? 'selected' : '' }}>
                            Arsip</option>
                        @if($donationItem->status === 'disalurkan')
                            <option value="disalurkan" selected>Sudah Disalurkan</option>
                        @endif
                    </select>
                    @error('status')
                        <p class="text-xs text-red-500 font-semibold mt-1">{{ $message }}</p>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div>
                        <label for="berat"
                            ...
                            (Gram)</label>
                        <input type="number" id="berat" name="berat" value="{{ old('berat', $donationItem->berat) }}"
                            min="0" placeholder="Contoh: 850 (untuk 0.85 kg)"
                            class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#22AF85]/20 focus:border-[#22AF85] text-sm text-gray-900 font-medium transition" />
                        @error('berat')
                            <p class="text-xs text-red-500 font-semibold mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="number" id="score_kelayakan" name="score_kelayakan"
                            value="{{ old('score_kelayakan', $donationItem->score_kelayakan) }}" min="0" max="100"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 border-t border-gray-100 pt-6">
                    ...
                    <div
                        ...
                        <template x-if="!utamaPreviewUrl">
                            <div class="w-full h-full relative">
                                <img src="{{ $donationItem->foto_utama_url }}" class="w-full h-full object-cover" />
                                <div
                                    class="absolute inset-0 bg-black/40 flex flex-col items-center justify-center text-white opacity-0 group-hover:opacity-100 transition duration-200">
                                    <span class="material-symbols-outlined !text-[32px] mb-1">cached</span>
                                    <p class="text-xs font-bold">Ganti Foto Utama</p>
                                </div>
                            </div>
                        </template>
                ...
                <div class="space-y-3">
                    <label class="block text-xs font-black text-gray-700 uppercase tracking-wider">Foto Detail
                        (Multi-Sudut)</label>
                    <p class="text-xs text-gray-400">Pilih foto untuk <strong>ditambahkan</strong> ke foto detail yang
                        sudah ada. Gunakan tombol ✕ di bawah untuk menghapus foto tertentu.</p>
                    ...
                    <div
                        ...
                        <div class="text-center p-4">
                            ...
                            <p class="text-xs font-bold text-gray-500 group-hover:text-[#22AF85]">Tambah Foto Detail</p>
                            <p class="text-[10px] text-gray-400 mt-1"
                                x-text="detailPreviews.length > 0 ? `${detailPreviews.length} foto baru siap ditambahkan` : 'Klik untuk pilih foto'">
                            </p>
                        </div>
                    </div>
                    @error('foto_detail')
            ...
            {{-- Grid gabungan: foto lama (bisa dihapus) + foto baru (preview) --}}
            <div class="border-t border-gray-100 pt-6">
                <template x-if="currentDetails.length > 0 || detailPreviews.length > 0">
                    <div>
                        <div class="flex items-center justify-between mb-3">
                            <p class="text-xs font-black text-gray-700 uppercase tracking-wider">
                                Foto Detail
                                <span class="ml-1 font-normal text-gray-400"
                                    x-text="`(${currentDetails.length + detailPreviews.length} foto)`"></span>
                            </p>
                            <p class="text-[10px] text-gray-400 font-semibold">Hover foto untuk hapus</p>
                        </div>
                        <div class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3">
                            ...
                            <template x-for="(url, index) in currentDetails" :key="'old-' + index">
                                <div
                                    class="relative rounded-xl border border-gray-200 aspect-square overflow-hidden group">
                                    <img :src="url"
                                        class="w-full h-full object-cover transition group-hover:brightness-75" />
                                    <button type="button" @click="removeCurrentDetail(index)"
                                    ...
                                    <span
                                        class="absolute bottom-1 left-1 bg-black/50 text-white text-[9px] font-bold px-1.5 py-0.5 rounded"
                                        x-text="'#' + (index + 1)"></span>
                                </div>
                            </template>
                            ...
                            <template x-for="(url, index) in detailPreviews" :key="'new-' + index">
                                <div
                                    ...
                                    {{-- Badge "Baru" untuk membedakan dari foto lama --}}
                                    <span
                                        class="absolute bottom-1 left-1 bg-[#22AF85] text-white text-[9px] font-bold px-1.5 py-0.5 rounded">Baru</span>
                                </div>
                            </template>

                        </div>
                        {{-- Notif foto yang akan dihapus --}}
                        <p x-show="deletedIndices.length > 0"
                            class="text-xs text-amber-600 font-semibold mt-3 flex items-center gap-1.5">
                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M12 9v2m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                            </svg>
                            <span x-text="`${deletedIndices.length} foto lama akan dihapus saat form disimpan.`"></span>
                        </p>
                    </div>
                </template>

                {{-- Hidden inputs untuk kirim index foto yang dihapus --}}
                <template x-for="idx in deletedIndices" :key="idx">
                    <input type="hidden" name="delete_detail[]" :value="idx">
                </template>
            </div>
    ...
    <script>
        function formApp() {
            return {
                ...
                detailPreviews: [],
                detailFiles: [],
                currentDetails: <?php echo json_encode(collect($donationItem->foto_detail)->map(function ($path) {
...
echo json_encode($servicesData);
                ...
                removeCurrentDetail(index) {
                    const originalIndices = <?php echo json_encode(array_keys(($donationItem->foto_detail ?? []))); ?>;
                    const originalIndex = originalIndices[index];

                    this.deletedIndices.push(originalIndex);
