...
@else
    <div class="grid grid-cols-2 md:grid-cols-2 xl:grid-cols-3 gap-3 md:gap-6">
        @foreach($items as $item)
            ...
            @endphp
            <div class="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-md group hover:-translate-y-0.5 transition-all duration-300 flex flex-col h-full border border-gray-100">
                <!-- Image -->
                <div class="relative overflow-hidden cursor-pointer bg-gray-50" @click="openDetail({{ $itemJson }})">
                ...
                <div class="p-3 sm:p-5 flex-grow flex flex-col justify-between">
                    <div>
                        <div class="flex justify-between items-center gap-2 mb-3">
                            ...
                            @if($item->status === 'tersedia')
                                <span class="px-2 py-0.5 bg-emerald-50 text-emerald-700 border border-emerald-100 rounded-full font-bold text-[9px] sm:text-[10px] tracking-wide uppercase">Tersedia</span>
                            @else
                        ...
                        <!-- Brand -->
                        <p class="text-emerald-600 font-bold text-[10px] sm:text-xs mb-3">
                            Brand: {{ $item->brand ?? 'Generic' }}
                        </p>
                    ...
                    <!-- Button Action -->
                    <div class="pt-2">
                        <button @click="openDetail({{ $itemJson }})" class="w-full py-2 sm:py-3 {{ $item->status === 'tersedia' ? 'bg-emerald-500 text-white hover:bg-emerald-600 active:scale-[0.98]' : 'bg-gray-100 text-gray-400 cursor-not-allowed' }} rounded-xl font-bold text-xs transition-all flex items-center justify-center gap-2" {{ $item->status !== 'tersedia' ? 'disabled' : '' }}>
